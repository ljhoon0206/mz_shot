...
# --- (이전 코드와 동일한 Mediapipe, Helper 함수들) ---
...
class PoseMatchProcessor(VideoProcessorBase):
    ...
    def recv(self, frame):
        ...
        if results and results.detections:
            # 1) 각 detection에 대해 각도/유사도/중심 x좌표 계산 및 저장
            temp_list = []
            ...
            for idx, info in enumerate(temp_list, start=1):
                ...
                # ... (시각적 피드백 로직 생략: 눈 선, 박스, 텍스트, 유사도 바 그리기) ...
...
def main():
    ...
    st.markdown(
        """
        **문제 해결:** 셔터 소리 지연 문제 해결을 위해, 캡처 시 신호를 **큐**에 넣어 메인 스레드에서 **`st.empty()` 컨테이너를 이용해 지속적으로 감지**하도록 수정했습니다. 이제 **캡처될 때마다** 소리가 납니다.
        ---
        """
    )
    ...
    # 캡처 횟수를 세션 상태에 저장하여 UI 업데이트 트리거로 사용
    if "capture_count" not in st.session_state:
        st.session_state["capture_count"] = 0

    # --- (타겟 사진 및 촬영 조건 설정 코드는 이전과 동일) ---
    ...
    # ⭐ 셔터 소리 트리거용 컨테이너
    shutter_trigger_container = st.empty()
    ...
    if webrtc_ctx.video_processor:
        ...
        try:
            # Non-blocking get
            if vp.shutter_queue.get(timeout=0.01):
                # 소리 재생
                shutter_trigger_container.components.html(SHUTTER_HTML, height=0)

                # 캡처 목록 업데이트를 위해 세션 상태에 복사
        ...
        # 캡처 횟수 상태를 키로 사용하여 목록 표시를 강제 업데이트
        if st.session_state["captured_images_main"]:
            st.button("캡처 목록 다시 그리기", key="refresh_ui")
