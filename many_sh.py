...
def main():
    ...
    # 🔔 Session State 초기화 (캡처 시간 추적용)
    if "prev_capture_time" not in st.session_state:
        st.session_state["prev_capture_time"] = 0.0
    if "show_captured_images" not in st.session_state:
        st.session_state["show_captured_images"] = False
    ...
    if webrtc_ctx.video_processor:
        ...
        if current_capture_time > st.session_state["prev_capture_time"]:
            ...
            # 캡처 목록이 바로 보이도록 상태 변경 (자동 새로고침 효과)
            st.session_state["show_captured_images"] = True
        ...
        if ref_angle is None:
            ...
            # ⚠️ 다음 번 Streamlit re-run을 위해 임의의 위젯을 사용하여 상태를 자주 업데이트함
            # (Streamlit이 스스로 재실행되도록 유도하는 방법 중 하나)
            # 웹캠이 활성화되어 있고, 얼굴이 감지되지 않으면 위젯을 숨김
            if not vp.person_infos:
                st.empty()  # 이 위젯은 화면에 표시되지 않지만, Streamlit을 re-run할 기회를 제공함
        ...
        # 캡처 목록 보기 토글
        st.session_state["show_captured_images"] = st.toggle(
            "캡처 목록 보기/숨기기",
            value=st.session_state["show_captured_images"],
        )

        if st.session_state["show_captured_images"] and vp.captured_images:
        ...
        elif st.session_state["show_captured_images"]:
