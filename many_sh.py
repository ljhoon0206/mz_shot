...
def main():
    ...
    if webrtc_ctx.video_processor:
        ...
        # 🔔 이전 캡처 개수 기억용
        if "last_capture_count" not in st.session_state:
            st.session_state.last_capture_count = 0

        current_count = len(vp.captured_images)

        # ✅ 새로 캡처된 사진이 생겼을 때만 소리 재생
        if current_count > st.session_state.last_capture_count:
            components.html(SHUTTER_HTML, height=0)

        st.session_state.last_capture_count = current_count
