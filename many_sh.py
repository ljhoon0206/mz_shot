    ...
    def recv(self, frame):
        ...
        if results and results.detections:
            ...
            for idx, info in enumerate(temp_list, start=1):
                ...
                ih, iw, _ = img.shape
                x, y, w_box, h_box = int(bbox.xmin * iw), int(bbox.ymin * ih), int(bbox.width * iw), int(bbox.height * ih)
...
def main():
    ...
    st.markdown(
        """
        **안내:** Stop 버튼을 누르면 촬영된 사진들이 아래에 즉시 표시됩니다.
        ---
        """
    )
    ...
    # =========================================================================
    # [2] 결과 갤러리 영역 (비디오 프로세서 유무와 상관없이 항상 실행되어야 함)
    # ⭐ 여기가 수정된 핵심 포인트입니다 (들여쓰기 제거)
    # =========================================================================
    st.markdown("---")
    st.subheader("자동 촬영된 사진들")

    if st.session_state["captured_images_main"]:
        st.button("캡처 목록 새로고침", key="refresh_ui")  # UI 갱신용
