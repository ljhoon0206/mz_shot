...
def main():
    ...
    st.markdown(
        ...
        **즉시 반영:** 셔터 소리 재생 후 `st.rerun()`을 호출하여 캡처된 이미지가 즉시 UI에 나타나도록 했습니다.
        ---
        """
    )
    ...
    if webrtc_ctx.video_processor:
        ...
        # 캡처 신호 감지 루프
        # Streamlit은 상태가 변경될 때마다 전체 스크립트를 다시 실행하기 때문에
        # 이 while 루프는 webrtc_ctx.state.playing이 True인 동안 계속 실행됩니다.
        # `st.rerun()` 호출 시 이 루프는 중단되고, 스크립트가 다시 시작됩니다.
        if webrtc_ctx.state.playing:
            try:
                # ⭐ 셔터 소리 신호 확인 (논블로킹)
                if not vp.shutter_queue.empty() and vp.shutter_queue.get(block=False):
                    ...
                    st.experimental_rerun() # st.rerun() 대신 st.experimental_rerun() 사용 (일부 버전에서)
        ...
        if st.session_state["captured_images_main"]:
            # st.button("캡처 목록 다시 그리기", key="refresh_ui") # st.rerun()으로 자동 갱신되므로 필요없음
